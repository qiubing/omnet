...
void Mac1609_4::initialize(int stage) {
	...
	if (stage == 0) {
		...
		myIndex = getParentModule()->getParentModule()->getIndex();
		//myMacAddress is linear relationship with myId, myMacAddress = 80000 + myId;
		//we only need to make sure the Mac address is unique;
		myMacAddress = 80000 + myIndex;

		DBG_MAC <<"myMacAddress is: "<< myMacAddress << "and myId is: "<< myId <<" and myIndex is: "<< myIndex << std::endl;
...
void Mac1609_4::handleSelfMsg(cMessage* msg) {
	if (msg == nextChannelSwitch) {
		...
		//set the dest MAC address, if rcvId is -1 then set the mac address as broadcast address,otherwise set the mac address to 80000+recvId
		//dest mac Address
		long dest;
		if(pktToSend->getNextForwardAddress() == -1)
		{
		    dest = LAddress::L2BROADCAST;
		}else
		{
		    dest = 80000 + pktToSend->getNextForwardAddress();
		}

		DBG_MAC << "##Mac1609_4:the dest Mac address is: " << dest << std::endl;
		mac->setDestAddr(dest);
...
void Mac1609_4::handleUpperMsg(cMessage* msg) {
	...
	if(std::string(thisMsg->getName()) == "wsm")
	{
	    mySCH = thisMsg->getChannelNumber();
	    ASSERT(mySCH);
	    if(mySCH == Channels::SCH1 || mySCH == Channels::SCH2 || mySCH == Channels::SCH3 || mySCH == Channels::SCH4)
	    {
	        ASSERT(useSCH);
	        chan = type_SCH;
	    }

	}else if(std::string(thisMsg->getName()) == "wsa")
	{
	    chan = type_CCH;
	}else
	{
	...
	if (num >= 1 && idleChannel == true && chan == activeChannel) {
		...
		if (nextEvent != -1) {
			if ((!useSCH) || (nextEvent <= nextChannelSwitch->getArrivalTime())) {
				...
				DBG_MAC << "##MAC_1609_4:the packet will send in channel: " << thisMsg->getChannelNumber() << "at the time: "<< nextEvent << std::endl;
